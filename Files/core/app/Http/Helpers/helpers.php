...
{
    $system['name'] = 'cryptocom';
    // $system['version'] = '3.1.39';
    $system['version'] = '1.0';
    return $system;
}
...
function showAmount($amount, $decimal = 8, $separate = true, $exceptZeros = false){
    $separator = '';
    if($separate){
        $separator = ',';
    }
...
function getRatingByUserProduct($u_id, $p_id){
    $rating = App\Models\Rating::where('user_id', $u_id)->where('product_id', $p_id)->first();
