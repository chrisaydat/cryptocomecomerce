...
class ManageUsersController extends Controller
{
    ...
    public function deposits(Request $request, $id)
    {
        ...
        $successful = $user->deposits()->where('status',1)->orderBy('id','desc')->sum('amount');
        $pending = $user->deposits()->where('status',2)->orderBy('id','desc')->sum('amount');
        $rejected = $user->deposits()->where('status',3)->orderBy('id','desc')->sum('amount');
